...
public class Client {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			TlsClientProtocol cproto = new TlsClientProtocol(SOCKET.getInputStream(), SOCKET.getOutputStream(),
					new SecureRandom());
							...
							try {
								X509Certificate serverCert = (X509Certificate) CertificateFactory.getInstance("X.509")
										.generateCertificate(new ByteArrayInputStream(
												serverCertificate.getCertificateList()[0].getEncoded()));
			...
			SOUT.writeObject(login);
			SOUT.flush();
			String loginreply = SIN.readUTF();
			System.out.println(loginreply);
			if (loginreply.contains("Login failed")) {
				close(0);
			}
			...
			while (true) {
				...
				if (cmd.equals("SFO")) {
					System.out.println(Utils.ANSI_RED + "Invalid Command. Type help to show available commands."
							+ Utils.ANSI_RESET);
					continue;
